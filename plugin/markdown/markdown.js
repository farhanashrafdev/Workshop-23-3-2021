...
function slidify(markdown, options) {
  ...
  var separatorRegex =
          new RegExp(options.separator + (options.verticalSeparator
                                              ? '|' + options.verticalSeparator
                                              : ''),
                     'mg'),
      horizontalSeparatorRegex = new RegExp(options.separator);

  var matches, lastIndex = 0, isHorizontal, wasHorizontal = true, content,
               sectionStack = [];
  ...
  (wasHorizontal ? sectionStack : sectionStack[sectionStack.length - 1])
  ...
  for (var i = 0, len = sectionStack.length; i < len; i++) {
    ...
    if (sectionStack[i] instanceof Array) {
      ...
      markdownSections += '<section ' + options.attributes + ' data-markdown>' +
...
function processSlides() {
  ...
  var sections = document.querySelectorAll('[data-markdown]'), section;
  ...
  for (var i = 0, len = sections.length; i < len; i++) {
    ...
    if (section.getAttribute('data-markdown').length) {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
          // file protocol yields status code 0 (useful for local debug, mobile
          // applications etc.)
  ...
  if (element != null && element.childNodes != undefined &&
    ...
    for (var i = 0; i < element.childNodes.length; i++) {
      ...
      if (typeof childElement.setAttribute == 'function' ||
        ...
        addAttributes(parentSection, childElement, previousParentElement,
                      separatorElementAttributes, separatorSectionAttributes);
      }
    }
  }
...
return {
  ...
  initialize : function() {
    ...
    if (typeof hljs !== 'undefined') {
      marked.setOptions({
        highlight : function(
            code, lang) { return hljs.highlightAuto(code, [ lang ]).value; }
      });
    }
