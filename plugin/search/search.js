 ...
 * occurrence to the user by navigatating to that slide and highlighting it.
 *
...
var RevealSearch = (function() {
    ...
    this.setRegex =
      ...
      if (node.nodeType == 3) { // NODE_TEXT
        if ((nv = node.nodeValue) && (regs = matchRegex.exec(nv))) {
          // find the slide's section element and save it in our list of
          // matching slides
          var secnode = node;
          ...
          for (var i = 0; i < slidelen; i++) {
            if ((matchingSlides[i].h === slideIndex.h) &&
  ...
  function doSearch() {
    ...
    if (matchedSlides) {
      // navigate to the next slide that has the keyword, wrapping to the first
      // if necessary
  ...
  document.getElementById('searchbutton')
      .addEventListener('click', function(event) { doSearch(); }, false);
  ...
  document.addEventListener('keydown', function(event) {
    if (event.key == "F" &&
        (event.ctrlKey || event.metaKey)) { // Control+Shift+f
      event.preventDefault();
      toggleSearch();
    }
  }, false);
