Reveal.addEventListener('ready', function() {
  ...
  QUnit.test('Vertical separator', function(assert) {
    assert.strictEqual(
        document.querySelectorAll('.reveal .slides>section>section').length, 4,
        'found four slides');
  });
  ...
  QUnit.test('Attributes on element header in vertical slides', function(
    ...
    assert.strictEqual(
        document
            ...
            .length,
        1, 'found one vertical slide with class fragment.fade-out on header');
    assert.strictEqual(
        document
            .querySelectorAll(
                '.reveal .slides section>section h2.fragment.shrink')
            .length,
        1, 'found one vertical slide with class fragment.shrink on header');
  });
  ...
  QUnit.test('Attributes on element paragraphs in vertical slides', function(
    ...
    assert.strictEqual(
        document
            .querySelectorAll('.reveal .slides section>section p.fragment.grow')
            .length,
        2,
