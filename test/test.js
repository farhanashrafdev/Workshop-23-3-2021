...
Reveal.addEventListener('ready', function() {
  ...
  QUnit.test('Initial slides classes', function(assert) {
    ...
    assert.strictEqual(
        document.querySelectorAll('.reveal .slides section.past').length, 0,
        'no .past slides');
    assert.strictEqual(
        document.querySelectorAll('.reveal .slides section.present').length, 1,
        'one .present slide');
    assert.strictEqual(
        document.querySelectorAll('.reveal .slides>section.future').length,
        horizontalSlides.length - 1, 'remaining horizontal slides are .future');

    assert.strictEqual(
        document.querySelectorAll('.reveal .slides section.stack').length, 2,
        'two .stacks');

    assert.ok(document.querySelectorAll('.reveal .slides section.stack')[0]
  ...
  QUnit.test('Reveal.isFirstSlide', function(assert) {
    Reveal.slide(0, 0);
    assert.strictEqual(Reveal.isFirstSlide(), true,
                       'true after Reveal.slide( 0, 0 )');

    Reveal.slide(1, 0);
    assert.strictEqual(Reveal.isFirstSlide(), false,
  ...
  QUnit.test('Reveal.isLastSlide', function(assert) {
    ...
    assert.strictEqual(Reveal.isLastSlide(), true,
  ...
  QUnit.test('Reveal.getSlideBackground', function(assert) {
    ...
    assert.strictEqual(Reveal.getSlideBackground(100), undefined,
                       'undefined when out of horizontal bounds');
    assert.strictEqual(Reveal.getSlideBackground(1, 100), undefined,
                       'undefined when out of vertical bounds');
  });

  QUnit.test('Reveal.getSlideNotes', function(assert) {
    Reveal.slide(0, 0);
    assert.ok(Reveal.getSlideNotes() === 'speaker notes 1',
              'works with <aside class="notes">');

    Reveal.slide(1, 0);
    assert.ok(Reveal.getSlideNotes() === 'speaker notes 2',
  ...
  QUnit.test('Reveal.getPreviousSlide/getCurrentSlide', function(assert) {
    ...
    var firstSlide =
        document.querySelector('.reveal .slides>section:first-child');
    var secondSlide =
        document.querySelector('.reveal .slides>section:nth-child(2)>section');
  ...
  QUnit.test('Reveal.configure', function(assert) {
    assert.strictEqual(Reveal.getConfig().loop, false,
                       '"loop" is false to start with');

    Reveal.configure({loop : true});
    assert.strictEqual(Reveal.getConfig().loop, true,
  ...
  QUnit.test('Hiding all fragments', function(assert) {
    var fragmentSlide =
        document.querySelector('#fragment-slides>section:nth-child(1)');

    Reveal.slide(2, 0, 0);
    assert.strictEqual(
        fragmentSlide.querySelectorAll('.fragment.visible').length, 1,
        'one fragment visible when index is 0');

    Reveal.slide(2, 0, -1);
    assert.strictEqual(
        fragmentSlide.querySelectorAll('.fragment.visible').length, 0,
  ...
  QUnit.test('Current fragment', function(assert) {
    ...
    Reveal.slide(2, 0);
    assert.strictEqual(
        fragmentSlide.querySelectorAll('.fragment.current-fragment').length, 0,
        'no current fragment at index -1');

    Reveal.slide(2, 0, 0);
    assert.strictEqual(
        fragmentSlide.querySelectorAll('.fragment.current-fragment').length, 1,
  ...
  QUnit.test('Stepping past fragments', function(assert) {
    ...
    Reveal.slide(0, 0, 0);
    assert.equal(fragmentSlide.querySelectorAll('.fragment.visible').length, 0,
                 'no fragments visible when on previous slide');

    Reveal.slide(3, 0, 0);
    assert.equal(fragmentSlide.querySelectorAll('.fragment.visible').length, 3,
                 'all fragments visible when on future slide');
  });

  QUnit.test('Fragment indices', function(assert) {
    var fragmentSlide =
        document.querySelector('#fragment-slides>section:nth-child(2)');

    Reveal.slide(3, 0, 0);
    assert.equal(fragmentSlide.querySelectorAll('.fragment.visible').length, 2,
                 'both fragments of same index are shown');

    // This slide has three fragments, first one is index 0, second and third
    // have index 1
    Reveal.slide(2, 2, 0);
    assert.equal(Reveal.getIndices().f, 0,
                 'returns correct index for first fragment');

    Reveal.slide(2, 2, 1);
    assert.equal(Reveal.getIndices().f, 1,
  ...
  QUnit.test('Index generation', function(assert) {
    ...
    assert.equal(fragmentSlide.querySelectorAll('.fragment')[0].getAttribute(
                     'data-fragment-index'),
                 '0');
    assert.equal(fragmentSlide.querySelectorAll('.fragment')[1].getAttribute(
                     'data-fragment-index'),
                 '1');
    assert.equal(fragmentSlide.querySelectorAll('.fragment')[2].getAttribute(
                     'data-fragment-index'),
                 '2');
  });
  ...
  QUnit.test('Index normalization', function(assert) {
    var fragmentSlide =
        document.querySelector('#fragment-slides>section:nth-child(3)');
  ...
  QUnit.test('Reveal.isAutoSliding', function(assert) {
    ...
    assert.strictEqual(Reveal.isAutoSliding(), false,
  ...
  QUnit.test('Reveal.toggleAutoSlide', function(assert) {
    ...
    Reveal.toggleAutoSlide();
    assert.strictEqual(Reveal.isAutoSliding(), true,
  ...
  QUnit.test('Controls', function(assert) {
    ...
    assert.equal(controlsElement.style.display, 'block',
  ...
  QUnit.test('Progress', function(assert) {
    ...
    assert.equal(progressElement.style.display, 'block',
  ...
  QUnit.test('img with data-src', function(assert) {
    assert.strictEqual(
        document.querySelectorAll('.reveal section img[src]').length, 1,
        'Image source has been set');
  });

  QUnit.test('video with data-src', function(assert) {
    assert.strictEqual(
        document.querySelectorAll('.reveal section video[src]').length, 1,
        'Video source has been set');
  });

  QUnit.test('audio with data-src', function(assert) {
    assert.strictEqual(
        document.querySelectorAll('.reveal section audio[src]').length, 1,
  ...
  QUnit.test('iframe with data-src', function(assert) {
    Reveal.slide(0, 0);
    assert.strictEqual(
        document.querySelectorAll('.reveal section iframe[src]').length, 0,
        'Iframe source is not set');
    Reveal.slide(2, 1);
    assert.strictEqual(
        document.querySelectorAll('.reveal section iframe[src]').length, 1,
