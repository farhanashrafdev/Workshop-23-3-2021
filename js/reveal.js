...
var SLIDES_SELECTOR = '.slides section',
    ...
    config = {
      ...
      // Use this method for navigation when auto-sliding (defaults to
      // navigateNext)
    ...
    overviewSlideWidth = null, overviewSlideHeight = null,
    ...
    hasNavigatedRight = false, hasNavigatedDown = false,
    ...
    autoSlidePlayer, autoSlideTimeout = 0, autoSlideStartTime = -1,
...
function load() {
  ...
  var scripts = [], scriptsAsync = [], scriptsToPreload = 0;
  ...
  function loadScript(s) {
    head.ready(s.src.match(/([\w\d_\-]*)\.?js(\?[\w\d.=&]*)?$|[^\\\/]*$/i)[0],
               function() {
...
function start() {
  ...
  setTimeout(function() {
    ...
    dispatchEvent(
        'ready',
        {'indexh' : indexh, 'indexv' : indexv, 'currentSlide' : currentSlide});
  }, 1);
...
function setupDOM() {
  ...
  // Progress bar
  dom.progress =
      createSingletonNode(dom.wrapper, 'div', 'progress', '<span></span>');
  dom.progressbar = dom.progress.querySelector('span');

  // Arrow controls
  dom.controls = createSingletonNode(
      dom.wrapper, 'aside', 'controls',
  ...
  dom.speakerNotes =
      createSingletonNode(dom.wrapper, 'div', 'speaker-notes', null);
  dom.speakerNotes.setAttribute('data-prevent-swipe', '');
  dom.speakerNotes.setAttribute('tabindex', '0');

  // Overlay graphic which is displayed during the paused mode
  dom.pauseOverlay = createSingletonNode(
      dom.wrapper, 'div', 'pause-overlay',
  ...
  // Element node
    ...
    if (isAriaHidden !== 'true' && !isDisplayHidden) {
      ...
      toArray(node.childNodes)
          .forEach(function(child) { text += getStatusText(child); });
    }
  }
...
function setupPDF() {
  ...
  var slideWidth = slideSize.width, slideHeight = slideSize.height;
  ...
  injectStyleSheet(
      ...
      slideWidth + 'px; max-height:' + slideHeight + 'px}');
  ...
  toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR))
      .forEach(function(hslide, h) {
        ...
        if (slide.classList.contains('stack') === false) {
          ...
          if (config.showNotes) {
            ...
            if (notes) {
              ...
              var notesSpacing = 8;
              var notesLayout = typeof config.showNotes === 'string'
                                    ? config.showNotes
                                    : 'inline';
          ...
          if (config.pdfSeparateFragments) {
            ...
            var fragmentGroups =
                sortFragments(page.querySelectorAll('.fragment'), true);
            ...
            fragmentGroups.forEach(function(fragments) {
              ...
              page.parentNode.insertBefore(clonedPage,
          ...
          // Show all fragments
            ...
            toArray(page.querySelectorAll('.fragment:not(.fade-out)'))
                .forEach(function(
                    fragment) { fragment.classList.add('visible'); });
          }
        }
      });
...
function createBackgrounds() {
  ...
  if (config.parallaxBackgroundImage) {
    ...
    // Make sure the below properties are set on the element - these properties
    // are needed for proper transitions to be set on the element via CSS. To
    // remove annoying background slide-in effect when the presentation starts,
    // apply these properties after short time delay
    setTimeout(
        function() { dom.wrapper.classList.add('has-parallax-background'); },
        1);
...
function syncBackground(slide) {
  ...
  if (data.background || data.backgroundColor || data.backgroundImage ||
      data.backgroundVideo || data.backgroundIframe) {
    element.setAttribute('data-background-hash',
                         data.background + data.backgroundSize +
                             data.backgroundImage + data.backgroundVideo +
                             data.backgroundIframe + data.backgroundColor +
                             data.backgroundRepeat + data.backgroundPosition +
  ...
  if (data.backgroundTransition)
    element.setAttribute('data-background-transition',
                         data.backgroundTransition);
...
function configure(options) {
  ...
  if (config.mouseWheel) {
    document.addEventListener('DOMMouseScroll', onDocumentMouseScroll,
                              false); // FF
  ...
  if (numberOfSlides > 1 && config.autoSlide && config.autoSlideStoppable &&
      features.canvas && features.requestAnimationFrame) {
    autoSlidePlayer = new Playback(dom.wrapper, function() {
      return Math.min(
          Math.max((Date.now() - autoSlideStartTime) / autoSlide, 0), 1);
    });
...
function addEventListeners() {
  ...
  pointerEvents.forEach(function(eventName) {
    dom.controlsLeft.forEach(function(
        el) { el.addEventListener(eventName, onNavigateLeftClicked, false); });
    dom.controlsRight.forEach(function(
        el) { el.addEventListener(eventName, onNavigateRightClicked, false); });
    dom.controlsUp.forEach(function(
        el) { el.addEventListener(eventName, onNavigateUpClicked, false); });
    dom.controlsDown.forEach(function(
        el) { el.addEventListener(eventName, onNavigateDownClicked, false); });
    dom.controlsPrev.forEach(function(
        el) { el.addEventListener(eventName, onNavigatePrevClicked, false); });
    dom.controlsNext.forEach(function(
        el) { el.addEventListener(eventName, onNavigateNextClicked, false); });
  });
}
...
function removeEventListeners() {
  ...
  ['touchstart', 'click'].forEach(function(eventName) {
    ...
    dom.controlsUp.forEach(function(
        el) { el.removeEventListener(eventName, onNavigateUpClicked, false); });
...
function enableRollingLinks() {
  ...
  if (features.transforms3d && !('msPerspective' in document.body.style)) {
    ...
    for (var i = 0, len = anchors.length; i < len; i++) {
      ...
      if (anchor.textContent && !anchor.querySelector('*') &&
...
function showPreview(url) {
  ...
  dom.overlay.innerHTML = [
    '<header>', '<a class="close" href="#"><span class="icon"></span></a>',
    '<a class="external" href="' + url +
        '" target="_blank"><span class="icon"></span></a>',
    '</header>', '<div class="spinner"></div>', '<div class="viewport">',
    '<iframe src="' + url + '"></iframe>', '<small class="viewport-inner">',
    '<span class="x-frame-error">Unable to load iframe. This is likely due to the site\'s policy (x-frame-options).</span>',
    '</small>', '</div>'
  ].join('');

  dom.overlay.querySelector('iframe').addEventListener(
      'load', function(event) { dom.overlay.classList.add('loaded'); }, false);
  ...
  dom.overlay.querySelector('.external')
      .addEventListener('click', function(event) { closeOverlay(); }, false);

  setTimeout(function() { dom.overlay.classList.add('visible'); }, 1);
}
...
function showHelp() {
  ...
  if (config.help) {
    ...
    for (var key in keyboardShortcuts) {
      html += '<tr><td>' + key + '</td><td>' + keyboardShortcuts[key] +
              '</td></tr>';
    }
    ...
    dom.overlay.innerHTML = [
      ...
      '<div class="viewport-inner">' + html + '</div>', '</div>'
    ].join('');
...
function layout() {
  ...
  if (dom.wrapper && !isPrintingPDF()) {
    ...
    if (!config.disableLayout) {
      ...
      scale = Math.min(size.presentationWidth / size.width,
...
function activateOverview() {
  ...
  if (config.overview && !isOverview()) {
    ...
    if (features.overviewTransitions) {
      setTimeout(function() { dom.wrapper.classList.add('overview-animated'); },
                 1);
    }
...
function layoutOverview() {
  ...
  toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR))
      .forEach(function(hslide, h) {
        hslide.setAttribute('data-index-h', h);
        transformElement(hslide, 'translate3d(' + (h * overviewSlideWidth) +
                                     'px, 0, 0)');
        ...
        if (hslide.classList.contains('stack')) {
          ...
          toArray(hslide.querySelectorAll('section'))
              .forEach(function(vslide, v) {
                ...
                transformElement(vslide, 'translate3d(0, ' +
                                             (v * overviewSlideHeight) +
                                             'px, 0)');
              });
        }
      });
  ...
  toArray(dom.background.childNodes).forEach(function(hbackground, h) {
    transformElement(hbackground,
                     'translate3d(' + (h * overviewSlideWidth) + 'px, 0, 0)');

    toArray(hbackground.querySelectorAll('.slide-background'))
        .forEach(function(vbackground, v) {
...
function deactivateOverview() {
  ...
  if (config.overview) {
    ...
    setTimeout(
        function() { dom.wrapper.classList.remove('overview-deactivating'); },
        1);
    ...
    toArray(dom.background.querySelectorAll('.slide-background'))
        .forEach(function(background) { transformElement(background, ''); });
...
function slide(h, v, f, o) {
  ...
  var indexhBefore = indexh || 0, indexvBefore = indexv || 0;

  // Activate and transition to the new slide
  indexh =
      updateSlides(HORIZONTAL_SLIDES_SELECTOR, h === undefined ? indexh : h);
  ...
  if (slidesLength) {
    ...
    for (var i = 0; i < slidesLength; i++) {
      ...
      if (i < index) {
        ...
        if (config.fragments) {
          var futureFragments =
              toArray(element.querySelectorAll('.fragment.visible'));
...
function updateSlidesVisibility() {
  ...
  var horizontalSlides =
          toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)),
      horizontalSlidesLength = horizontalSlides.length, distanceX, distanceY;
    ...
    for (var x = 0; x < horizontalSlidesLength; x++) {
      ...
      if (verticalSlidesLength) {
        ...
        for (var y = 0; y < verticalSlidesLength; y++) {
          ...
          distanceY = x === (indexh || 0) ? Math.abs((indexv || 0) - y)
                                          : Math.abs(y - oy);
...
function updateControls() {
  ...
  // Add the 'enabled' class to the available routes; remove 'disabled'
  // attribute to enable buttons
  if (routes.left)
    dom.controlsLeft.forEach(function(el) {
...
function updateBackground(includeAll) {
  ...
  toArray(dom.background.childNodes).forEach(function(backgroundh, h) {
    ...
    if (includeAll || h === indexh) {
      toArray(backgroundh.querySelectorAll('.slide-background'))
          .forEach(function(backgroundv, v) {
  ...
  setTimeout(function() { dom.background.classList.remove('no-transition'); },
             1);
}
...
function updateParallax() {
  ...
  if (config.parallaxBackgroundImage) {
    ...
    var slideHeight = dom.background.offsetHeight,
        verticalSlideCount = verticalSlides.length, verticalOffsetMultiplier,
        verticalOffset;
  ...
  if (background) {
    ...
    if (background.hasAttribute('data-loaded') === false) {
      ...
      // Videos
        ...
        backgroundVideo.split(',').forEach(function(
            source) { video.innerHTML += '<source src="' + source + '">'; });
...
function unloadSlide(slide) {
  ...
  toArray(slide.querySelectorAll(
...
function availableFragments() {
  ...
  if (currentSlide && config.fragments) {
    ...
    var hiddenFragments =
        currentSlide.querySelectorAll('.fragment:not(.visible)');
...
function formatEmbeddedContent() {
  ...
  var _appendParamToIframeSource = function(sourceAttribute, sourceURL, param) {
    toArray(dom.slides.querySelectorAll('iframe[' + sourceAttribute + '*="' +
                                        sourceURL + '"]'))
        .forEach(function(el) {
          var src = el.getAttribute(sourceAttribute);
          if (src && src.indexOf(param) === -1) {
            el.setAttribute(sourceAttribute,
                            src + (!/\?/.test(src) ? '?' : '&') + param);
          }
        });
  };
  ...
  if (isMobileDevice) {
    toArray(dom.slides.querySelectorAll('video, audio'))
        .forEach(function(el) { el.controls = true; });
  }
}
  ...
  if (element && !isSpeakerNotes()) {
    ...
    toArray(element.querySelectorAll('iframe[data-src]')).forEach(function(el) {
      ...
      if (el.getAttribute('src') !== el.getAttribute('data-src')) {
        el.removeEventListener(
            'load', startEmbeddedIframe); // remove first to avoid dupes
...
function startEmbeddedIframe(event) {
  ...
  if (iframe && iframe.contentWindow) {
    ...
    if (isAttachedToDOM && isVisible) {
      ...
      // own autoplay setting
      if (typeof autoplay !== 'boolean') {
        autoplay = iframe.hasAttribute('data-autoplay') ||
...
function stopEmbeddedContent(element, options) {
  ...
  if (element && element.parentNode) {
    ...
    // YouTube postMessage API
    toArray(element.querySelectorAll('iframe[src*="youtube.com/embed/"]'))
        .forEach(function(el) {
          if (!el.hasAttribute('data-ignore') && el.contentWindow &&
    ...
    // Vimeo postMessage API
    toArray(element.querySelectorAll('iframe[src*="player.vimeo.com/"]'))
        .forEach(function(el) {
          if (!el.hasAttribute('data-ignore') && el.contentWindow &&
              typeof el.contentWindow.postMessage === 'function') {
...
function readURL() {
  ...
  var bits = hash.slice(2).split('/'), name = hash.replace(/#|\//gi, '');
  ...
  if (!/^[0-9]*$/.test(bits[0]) && name.length) {
    ...
    var isSameNameAsCurrentSlide =
        currentSlide ? currentSlide.getAttribute('id') === name : false;
...
function getSlide(x, y) {
  ...
  var horizontalSlide =
      dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)[x];
  var verticalSlides =
 ...
 * time.
 *
 * @return {{indexh: number, indexv: number, indexf: number, paused: boolean,
 *     overview: boolean}}
 */
function getState() {
...
function setState(state) {
  ...
  if (typeof state === 'object') {
    slide(deserialize(state.indexh), deserialize(state.indexv),
    ...
    if (fragments.length) {
      ...
      if (typeof index !== 'number') {
        var lastVisibleFragment =
            sortFragments(currentSlide.querySelectorAll('.fragment.visible'))
                .pop();
      ...
      var fragmentsShown = [], fragmentsHidden = [];
      ...
      if (fragmentsHidden.length) {
        dispatchEvent(
            'fragmenthidden',
            {fragment : fragmentsHidden[0], fragments : fragmentsHidden});
      }

      if (fragmentsShown.length) {
        dispatchEvent(
            'fragmentshown',
            {fragment : fragmentsShown[0], fragments : fragmentsShown});
      }
...
function cueAutoSlide() {
  ...
  if (currentSlide && config.autoSlide !== false) {
    ...
    if (!fragment)
      fragment = currentSlide.querySelector('.fragment');

    var fragmentAutoSlide =
        fragment ? fragment.getAttribute('data-autoslide') : null;
    var parentAutoSlide =
    ...
    if (currentSlide.querySelectorAll('.fragment').length === 0) {
      toArray(currentSlide.querySelectorAll('video, audio'))
          .forEach(function(el) {
    ...
    if (autoSlide && !autoSlidePaused && !isPaused() && !isOverview() &&
        (!Reveal.isLastSlide() || availableFragments().next ||
         config.loop === true)) {
      autoSlideTimeout = setTimeout(function() {
        typeof config.autoSlideMethod === 'function' ? config.autoSlideMethod()
                                                     : navigateNext();
        cueAutoSlide();
      }, autoSlide);
...
function navigatePrev() {
  ...
  if (previousFragment() === false) {
    if (availableRoutes().up) {
      ...
      if (config.rtl) {
        previousSlide = toArray(dom.wrapper.querySelectorAll(
...
function onDocumentKeyDown(event) {
  ...
  if (activeElementIsCE || activeElementIsInput || activeElementIsNotes ||
      (event.shiftKey && event.keyCode !== 32) || event.altKey ||
  ...
  if (triggered === false) {
    ...
    case 13:
      ...
      break;
    // two-spot, semicolon, b, v, period, Logitech presenter tools "black
    // screen" button
    case 58:
    case 59:
    case 66:
...
function onTouchStart(event) {
  ...
  if (isSwipePrevented(event.target))
    return true;
...
function onTouchMove(event) {
  ...
  if (!touch.captured) {
    ...
    // There was only one touch point, look for a swipe
      ...
      var deltaX = currentX - touch.startX, deltaY = currentY - touch.startY;

      if (deltaX > touch.threshold && Math.abs(deltaX) > Math.abs(deltaY)) {
        touch.captured = true;
        navigateLeft();
      } else if (deltaX < -touch.threshold &&
...
function onProgressClicked(event) {
  ...
  var slidesTotal =
      toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)).length;
  var slideIndex =
      Math.floor((event.clientX / dom.wrapper.offsetWidth) * slidesTotal);
...
Playback.prototype.render = function() {
  var progress = this.playing ? this.progress : 0,
      radius = (this.diameter2) - this.thickness, x = this.diameter2,
      y = this.diameter2, iconSize = 28;
...
Playback.prototype.on = function(
    type, listener) { this.canvas.addEventListener(type, listener, false); };

Playback.prototype.off = function(
    type, listener) { this.canvas.removeEventListener(type, listener, false); };
...
Reveal = {
  ...
  getPreviousSlide : function() { return previousSlide; },
  ...
  getQueryHash : function() {
    ...
    location.search.replace(
        /[A-Z0-9]+?=([\w\.%-]*)/gi,
        function(a) { query[a.split('=').shift()] = a.split('=').pop(); });
  ...
  isFirstSlide : function() { return (indexh === 0 && indexv === 0); },
  ...
  triggerKey : function(keyCode) { onDocumentKeyDown({keyCode : keyCode}); },

  // Registers a new shortcut to include in the help overlay
  registerKeyboardShortcut : function(key,
                                      value) { keyboardShortcuts[key] = value; }
};
